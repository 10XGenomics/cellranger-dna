...
def get_dpcvs( M, mad_score = 6 ):
    ...
    outlier_mask = numpy.array([numpy.abs(M[i] - median[i]) < mad_score*mads[i] 
                             for i in xrange(M.shape[0])]).astype(bool)
...
def get_local_dpcvs( M, dpcv_per_cell, coarse_binning_factor=10, lpop_window_size=50 ):
    """Inputs:
       M             - ncell x nbin matrix of counts
       dpcv_per_cell - ncell array of dpcv estimates
       coarse_binning_factor - correction for estimating dpcv and diff_dpcv at different
                               bin sizes
       lpop_window_size - sliding window size for estimating lpop

    :returns: tuple (lpop, local_dpcv) 
    """
    ##
    ## calculates sliding window estimate of local poissonness
    ## using the observation that the std dev of the diff
    ## process should have std. dev. == sqrt(2)*mu and should
    ## be robust to discontinuous jumps in the mean (CNVs)
    ##
    def median_windowed_diff_dpcv( data, window_size=50 ):
        n_windows = int(math.floor(float(len(data))/float(window_size)))
        trimmed_data = data[:(n_windows*window_size)]
        data2 = numpy.reshape(trimmed_data,(n_windows,window_size))
        ddiff = numpy.diff(data2,axis=1)
        mu = data2.mean(axis=1)
        diff_dpcv = ddiff.std(axis=1) / numpy.sqrt(2.0*mu) - 1.0
        return numpy.median(diff_dpcv) / math.sqrt(numpy.median(mu))

    lpop = numpy.zeros(len(M),dtype='float32')
    for i in xrange(len(M)):
        lpop[i] = median_windowed_diff_dpcv(M[i],window_size=lpop_window_size)

    delta_hat = lpop / math.sqrt(coarse_binning_factor)
    mu = M.mean(axis=1) / coarse_binning_factor
    local_dpcv0 = math.sqrt(coarse_binning_factor) * \
        numpy.sqrt( delta_hat*delta_hat + 2.0 * delta_hat / numpy.sqrt(mu) )

    ## at this point the estimate of local_dpcv is excellent for "flat" data but
    ## but will have some slope bias for CNV data
    ## Empirically it's been seen that dpcv ~ (1-chi)*local_dpcv0 + chi
    ## where chi is an unknown parameter representing the dpcv due to CNV content
    ## This allows us to estimate the residual CNV term with a one-parameter estimate
    cnv_factors = (1.0-dpcv_per_cell)/(1.0-local_dpcv0)
    cnv_factors[ numpy.isinf(cnv_factors) | numpy.isnan(cnv_factors) ] = 1.0
    cnv_factor = numpy.median(cnv_factors)

    return lpop, cnv_factor*local_dpcv0
