...
from crdna.constants import MAX_READ_LENGTH
...
def combine_summary_managers(outs):
    ...
    for out in outs:
        ...
        if misc_sm is None:
            ...
            misc_sm = misc_sm.combine( tmp_sm )
    return misc_sm
...
def combine_nested_summary_managers(outs):
    ...
    for out in outs:
        ...
        for key in tmp_dict:
            tmp_sm = tmp_dict[key]
            if not(key in qual_sms):
                qual_sms[key] = tmp_sm
            else:
                qual_sms[key] = qual_sms[key].combine( tmp_sm )
    return qual_sms
...
def join(args, outs, chunk_defs, chunk_outs):
    ...
    # logging
    print tenkit.safe_json.safe_jsonify(misc_sm.get_summarizer('metrics').dict, pretty=True)
...
def compute_basic_stats(bam_in, target_regions, n_mapped, references,
    ...
    insert_sizes_hist = tk_summary.DiscreteDistribution(1, MAX_INSERT_SIZE)
    r1_len_hist = tk_summary.DiscreteDistribution(1, MAX_READ_LENGTH)
    r2_len_hist = tk_summary.DiscreteDistribution(1, MAX_READ_LENGTH)
    ...
    for info in read_pair_info_iter(bam_in, target_regions, references):
        ...
        if not(r1_mapped and r2_mapped):
            ...
            if info['r1_chrom'] == info['r2_chrom']:
                ...
                if info['r1_seq_len'] <= MAX_READ_LENGTH:
                    r1_len_hist.add(info['r1_seq_len'])

                if info['r2_seq_len'] <= MAX_READ_LENGTH:
...
def read_pair_info_iter(bam_in, target_regions, references):
    ...
    while(True):
        ...
        read_pair_info['r1_q20_bases'] = tk_fasta.get_bases_qual(read1.qual, 20)
        read_pair_info['r1_q30_bases'] = tk_fasta.get_bases_qual(read1.qual, 30)

        read_pair_info['r2_q20_bases'] = tk_fasta.get_bases_qual(read2.qual, 20)
        read_pair_info['r2_q30_bases'] = tk_fasta.get_bases_qual(read2.qual, 30)
