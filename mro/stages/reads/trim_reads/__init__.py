...
def split(args):
    chunks = [{'chunk': c, '__threads': 2} for c in args.chunks]
...
def join(args, outs, chunk_defs, chunk_outs):
    ...
    if bc_error_rate > 0.97:
        martian.exit("Extremely high rate of incorrect barcodes observed (%.2f %%). Check that input is 10x Chromium data, and that there are no missing cycles in the first 16bp of Read 1." % (bc_error_rate * 100.0))
    ...
    if lot_map is not None:
        ...
        bc_seq = sorted(list(bc_utils.load_barcode_whitelist(args.barcode_whitelist)))
...
def main(args, outs):
    ...
    if args.barcode_whitelist is None:
        ...
        barcode_whitelist = sorted(list(bc_utils.load_barcode_whitelist(args.barcode_whitelist)))
